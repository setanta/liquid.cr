...
module Liquid
  ...
  struct StackMachine
    ...
    # FIXME: What about string escapes?
    private def parse_string(scanner : StringScanner, limit : Char) : String
      regex = case limit
              when '"'  then /(?=")/
              when '\'' then /(?=')/
              else
                raise Exception.new("Unknown string limit #{limit}.")
              end

      scanner.offset += 1
      value = scanner.scan_until(regex)
      raise Exception.new("Unterminated string literal.") if value.nil?
